      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item label="审核节点">
            <mj-select v-model="search.productType" :kind="this.$enum.LOAN_KIND" :group="this.$enum.PRODUCT"
  ...
  export default {
      ...
      return {
        ...
        table: [
          ...
          },
          {
            label: "签收人",
            prop: "operatorName",
            formatter: (row, col, val) => (val == "" || val == null ? "--" : val)
          }
        ],
        reformModel: {
          partyId: ''
    ...
    methods: {
      ...
      getData(pageSize, pageNum) {
        ...
        getCreditAllList(search).then(({data}) => {
          if (data.code === 200) {
      ...
      handleSave() {
        this.$refs.reformModel.validate(val => {
          if (val) {
            this.creditReformSchool(this.applicationId,this.reformModel.partyId);
          }else {
            return false;
          }
        })
      ...
      creditReformSchool(applicationId,auditPartyId){
        creditReformSchool(applicationId,auditPartyId).then(response => {
          var res = response.data;
          if (res.code == 200){
            this.$message({
              type: "success",
