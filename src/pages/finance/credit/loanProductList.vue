...
<template>
  <div class="box">
    <el-form :model="search" ref="searchForm" class="form-label flex fixed-width">
      <el-row>
        <el-col :span="6">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="产品状态" prop="status">
              <mj-select v-model="product.status" auto-complete="off" :mode="'VIEW'" :kind="this.$enum.PRODUCT_STATUS" :group="this.$enum.PRODUCT_STATUS"></mj-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="产品名称" prop="productName">
              <mj-input v-model="product.productName" auto-complete="off" :mode="'VIEW'"></mj-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="产品编号" prop="productId">
              <mj-input v-model="product.productId" auto-complete="off" :mode="'VIEW'"></mj-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="放款类型" prop="paymentType">
              <mj-radio v-model="product.paymentType" :disabled="mode !=='EDIT'"
                        :kind="this.$enum.LOAN_TYPE" :group="this.$enum.LOAN_TYPE"
                        :sequence=[0,2]
                        @change="repayChange"></mj-radio>
            </el-form-item>
          </el-col>
        </el-row>
        <!--第三方放款(北京)存管(THIRD_PARTY_LENDING)和西安存管(XIAN_PARTY_LENDING)不需要展示-->
        <el-row v-if="product.paymentType =='FINANCIAL_LENDING'">
          <el-col :span="12">
            <el-form-item label="财务放款类型" prop="financePaymentType">
              <mj-radio v-model="product.financePaymentType" :disabled="mode !=='EDIT'" :kind="this.$enum.FINANCIAL_TYPE" :group="this.$enum.FINANCIAL_TYPE"
        ...
        <el-row v-if="product.paymentType=='FINANCIAL_LENDING' && product.financePaymentType=='AUTOMATIC'">
          <el-col :span="12">
            <el-form-item label="自动放款单笔限额" prop="paymentSingleLimit">
              <mj-input v-model="product.paymentSingleLimit" :disabled="mode!=='EDIT'" unit="元"></mj-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row v-if="product.paymentType=='FINANCIAL_LENDING' && product.financePaymentType=='AUTOMATIC'">
          <el-col :span="12">
            <el-form-item label="自动放款产品限额" prop="paymentProductLimit">
              <mj-input v-model="product.paymentProductLimit" :disabled="mode!=='EDIT'" unit="元/日"></mj-input>
            </el-form-item>
          </el-col>
        </el-row>
        <!--西安存管和第三方放款(北京)存管不需要展示-->
        <el-row v-if="product.paymentType=='FINANCIAL_LENDING'">
          <el-col :span="12">
            <el-form-item label="资金账户提现方式" prop="withdrawMethod">
              <mj-radio v-model="product.withdrawMethod" :disabled="mode !=='EDIT'" :kind="this.$enum.WITHDRAW_TYPE" :group="this.$enum.WITHDRAW_TYPE"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="还款方式" prop="repayType">
              <mj-radio v-model="product.repayType" :disabled="mode!=='EDIT'" :kind="this.$enum.REPAYMENT_TYPE" :group="this.$enum.REPAYMENT_TYPE" @change="wayChange"></mj-radio>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <div slot="footer">
        <el-button v-if="mode=='EDIT'" type="primary" @click="handleSave">保 存</el-button>
...
<script>
  ...
  export default {
    ...
    created() {
      this.getManageData(this.search.pageSize,this.search.pageNumber)
    },
    methods: {
      handleCurrentChange(val){
        this.search.pageNumber = val
        this.getManageData(this.search.pageSize,val);
      },
      handleSizeChange(val){
        this.search.pageSize = val
        this.getManageData(val,this.search.pageNumber)
      },
      // 查询列表
      handleSearch() {
      ...
      getManageData(pageSize,pageNum) {
      ...
      handleSave() {
        this.$refs.product.validate((valid) => {
          if (valid) {
            if(this.product.paymentType=="THIRD_PARTY_LENDING" || this.product.paymentType=="XIAN_PARTY_LENDING"){
