      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="还款日" prop="repayDay">
            <mj-radio v-model="product.repayDay" :kind="this.$enum.REPAY_DAY" :group="this.$enum.PRODUCT_STATUS" @change="changeRepayDay"
      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="计息方式" prop="interestWay">
            <mj-radio v-model="product.interestWay" :kind="this.$enum.INTEREST_RATE" :group="this.$enum.PRODUCT_STATUS" @change="changeTnterestWay"
              :disabled="mode !='CREATE'">
            </mj-radio>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="还款方式" prop="repayWay">
            <mj-radio v-model="product.repayWay" :kind="this.$enum.REPAY_WAY" :group="this.$enum.REPAY_WAY" @change="changeRepayWay"
              :disabled="mode !='CREATE'">
            </mj-radio>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col :span="15">
          <el-form-item label="提前还款违约金计算方式" prop="defaultsCalMethod">
            <mj-radio v-model="product.defaultsCalMethod" :kind="this.$enum.DEFAULTS_CAL_METHOD" :group="this.$enum.PRODUCT_STATUS" @change="changeDfCalMethod"
              :disabled="mode !='CREATE'">
            </mj-radio>
          </el-form-item>
        </el-col>
            ...
            <template slot-scope="scope">
              <el-form-item v-if="scope.row.$create || scope.row.$edit" :prop="scope.row[col.prop]">
                <div v-if="col.type == 'input'">
                  <mj-input v-if="col.isBlur" v-model.trim="scope.row[col.prop]" :mode="col.readyOnly ? 'VIEW': 'CREATE'" @blur="changeMonthRate(scope.row[col.prop],scope.row)"></mj-input>
                  <mj-input v-else v-model.trim="scope.row[col.prop]" :mode="col.readyOnly ? 'VIEW': 'CREATE'"></mj-input>
                </div>
                <div v-else-if="col.type === 'select'">
                  <mj-select v-if="col.kind" v-model="scope.row[col.prop]" :kind="col.kind" :group="col.group" :mode="mode"></mj-select>
                </div>
      ...
      changeMonthRate(val, row) {
        if (val) {
          if (row['baseIntRatioOne']) {
            if (row['baseIntRatioOne'] < 0) {
            ...
            } else if (!/^([0-9][0-9]*(\.[0-9]{1,2})?)$/.test(row['baseIntRatioOne'])) {
              this.lowFlag = false;
              row['baseIntRatioOne'] = "";
              this.$message({
                type: 'warning',
              ...
              this.lowFlag = true;
              this.nullFlag = true;
              row['baseIntRatioOne'] = parseFloat(row['baseIntRatioOne']).toFixed(2).toString();
            }
          }
          if (row['baseIntRatioTwo']) {
            if (row['baseIntRatioTwo'] < 0) {
            ...
            } else if (!/^([0-9][0-9]*(\.[0-9]{1,2})?)$/.test(row['baseIntRatioTwo'])) {
              this.lowFlag = false;
              row['baseIntRatioTwo'] = "";
              this.$message({
                type: 'warning',
              ...
              this.lowFlag = true;
              this.nullFlag = true;
              row['baseIntRatioTwo'] = parseFloat(row['baseIntRatioTwo']).toFixed(2).toString();
            }
          }
          if (row['manageIntRatioOne']) {
            if (row['manageIntRatioOne'] < 0) {
            ...
            } else if (!/^([0-9][0-9]*(\.[0-9]{1,2})?)$/.test(row['manageIntRatioOne'])) {
              this.lowFlag = false;
              row['manageIntRatioOne'] = "";
              this.$message({
                type: 'warning',
              ...
              this.lowFlag = true;
              this.nullFlag = true;
              row['manageIntRatioOne'] = parseFloat(row['manageIntRatioOne']).toFixed(2).toString();
            }
          }
          if (row['manageIntRatioTwo']) {
            if (row['manageIntRatioTwo'] < 0) {
            ...
            } else if (!/^([0-9][0-9]*(\.[0-9]{1,2})?)$/.test(row['manageIntRatioTwo'])) {
              this.lowFlag = false;
              row['manageIntRatioTwo'] = "";
              this.$message({
                type: 'warning',
              ...
              this.lowFlag = true;
              this.nullFlag = true;
              row['manageIntRatioTwo'] = parseFloat(row['manageIntRatioTwo']).toFixed(2).toString();
            }
          }
          if (row['baseIntRatioOne'] && row['baseIntRatioTwo'] && (row['baseIntRatioOne'] > 0 || row['baseIntRatioTwo'] >
              0)) {
            row['baseIntRatioOne'] = parseFloat(row['baseIntRatioOne']).toFixed(2).toString();
            row['baseIntRatioTwo'] = parseFloat(row['baseIntRatioTwo']).toFixed(2).toString();
            row['baseIntRatioExt'] = (parseFloat(row['baseIntRatioOne']) + parseFloat(row['baseIntRatioTwo'])).toFixed(
              2).toString();
          } else if (parseFloat(row['baseIntRatioOne']) == 0 && parseFloat(row['baseIntRatioTwo']) == 0) {
            row['baseIntRatioExt'] = "";
            this.zeroFlag = false;
            this.$message({
              type: 'warning',
              message: '月基本费率1和月基本费率2不能同时为0'
            });
          }
          if (row['manageIntRatioOne'] && row['manageIntRatioTwo'] && (row['manageIntRatioOne'] > 0 || row[
              'manageIntRatioTwo'] > 0)) {
            row['manageIntRatioOne'] = parseFloat(row['manageIntRatioOne']).toFixed(2).toString();
            row['manageIntRatioTwo'] = parseFloat(row['manageIntRatioTwo']).toFixed(2).toString();
            row['manageIntRatioExt'] = (parseFloat(row['manageIntRatioOne']) + parseFloat(row['manageIntRatioTwo'])).toFixed(
              2).toString();
          ...
          if (row['baseIntRatioExt'] && row['manageIntRatioExt']) {
            this.zeroFlag = true;
            row['monthFeeRate'] = (parseFloat(row['baseIntRatioExt']) + parseFloat(row['manageIntRatioExt'])).toFixed(2)
              .toString();
          }
          ...
          if (row['baseIntRatioOne'] == "" || row['baseIntRatioTwo'] == "") {
            row['baseIntRatioExt'] = "";
            row['monthFeeRate'] = "";
          }
      ...
      getStrategyDetail(id) {
        getStrategyDetail(id).then(response => {
          ...
          if (res.code === 200) {
            ...
            this.product.penaltyRate = this.$floatMultiply(res.body.penaltyRate, 100);
            this.product.penaltyRatio = this.$floatMultiply(res.body.penaltyRatio, 100);
            this.product.baseIntRatio = this.$floatMultiply(res.body.baseIntRatio, 100);
            this.product.defaultsRate = this.$floatMultiply(res.body.defaultsRate, 100);
            ...
            if (res.body.repayWay === 'QI') {
              this.flag = true;
              this.product.repayRatio = this.$floatMultiply(this.product.repayRatio, 100);
            } else {
              this.flag = false;
            }
          }
